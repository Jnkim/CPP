...
void showData(struct Address *st); // (Call by Address )
...
int main(){
   ...
   showData(&adr); // 구조체 포인터주소값을 전달 
   return 0;	
	
}

void showData(struct Address *st){ // 구조체 포인터값을 전달받는 변수 "*변수" 
    // 값을 수정하면 원본(struct)의 값도 변경된다. 
    
	cout << "이름 : "<< st->name << endl;   // 구조체 값참조 방식 ( Address 나 값참조일경우: "구조체명->속성") 
